/* ===== CONFIG ===== */
const API_BASE = "https://api.sparkedservers.us"; // <- tu daj swój reverse proxy do backendu
...
const saveCart = () => localStorage.setItem("ds_cart", JSON.stringify(cart));
...
function updateUserBox() {
  ...
  const ap = $("#adminPanel");
  if (ap) ap.toggleAttribute("hidden", !(currentUser && currentUser.is_admin));
}
...
async function doOtpLogin() {
  ...
  // jeśli Twój sklep to index.html:
  location.href = "./index.html";
  // jeśli sklep to shop.html — zamień na: location.href = "./shop.html";
}
...
function updateCartIcon() {
  const cnt = cart.reduce((a, c) => a + (c.qty || 0), 0);
  const el = $("#cartCount"); if (el) el.textContent = String(cnt);
}
...
function renderGrid() {
  ...
  for (const p of products) {
    ...
    $(".add", card).onclick = () => {
      if (!sel.options.length) return alert("Brak wariantów dla produktu.");
      const lab = sel.value;
      const price = Number(sel.selectedOptions[0]?.dataset.price || 0);
      const link  = sel.selectedOptions[0]?.dataset.link || "";
      const idx = cart.findIndex(it => it.id === p.id && it.option?.label === lab);
      if (idx > -1) cart[idx].qty += 1;
      else cart.push({ id: p.id, title: p.title, qty: 1, option: { label: lab, price, link } });
...
function bindAdmin() {
  ...
  if ($("#optList")?.children.length === 0) addOptRow("1 miesiąc", "10.00", "");
  ...
  $("#productForm")?.addEventListener("submit", async (e) => {
    ...
    if (!id || !title) return alert("Podaj ID i Tytuł.");
    ...
    if (!options.length) options.push({ label: "Standard", price: 0, link: "" });
...
function openCart() {
  ...
  list.onclick = (e) => {
    ...
    saveCart(); openCart(); updateCartIcon();
  };
