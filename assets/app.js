/* ===== CONFIG ===== */
const API_BASE = "https://api.sparkedservers.us"; // <— PODMIEŃ na swój reverse proxy (np. https://api.sparkedservers.us:8091)
...
function renderGrid() {
  ...
  for (const p of products) {
    ...
    for (const o of p.options || []) {
      ...
      opt.value = o.label;
      opt.textContent = `${o.label} — ${fmt(o.price)}`;
    ...
    $(".add", card).onclick = () => {
      ...
      cart.push({ id: p.id, title: p.title, qty: 1, option: { label: lab, price } });
      updateCartCount();
    };
...
function openCart() {
  ...
  cart.forEach((it, idx) => {
    ...
    row.innerHTML = `
      ...
      <div class="price">${fmt(it.option.price)}</div>
